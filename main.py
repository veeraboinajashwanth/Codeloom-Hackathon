...
symptoms_dict = {'itching': 0, 'skin_rash': 1, 'nodal_skin_eruptions': 2, 'continuous_sneezing': 3,
                 ...
                 'enlarged_thyroid': 71, 'brittle_nails': 72, 'swollen_extremities': 73,
                 ...
                 'fluid_overload.1': 117, 'blood_in_sputum': 118,
...
diseases_list = {15: 'Fungal infection', 4: 'Allergy', 16: 'GERD', 9: 'Chronic cholestasis',
                 ...
                 0: '(vertigo) Paroxysmal Positional Vertigo', 2: 'Acne',
...
def get_predicted_value(user_symptoms):
    ...
    for item in user_symptoms:
        # Check if the item is a valid key in symptoms_dict
        if item in symptoms_dict:
            input_vector[symptoms_dict[item]] = 1
        else:
            # Handle the case where the item is not in symptoms_dict
            print(f"Warning: Symptom '{item}' not found in symptoms_dict")

    # Print input_vector and prediction for debugging
    print("Input Vector:", input_vector)
    prediction = svc.predict([input_vector])[0]
    print("Prediction:", prediction)

    # Reset input_vector to zeros for the next prediction
    input_vector = [0] * len(symptoms_dict)
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms":
            ...
            valid_symptoms = all(symptom in symptoms_dict for symptom in user_symptoms)

            if not valid_symptoms:
                message = "Invalid symptoms provided. Please check the symptoms and try again."
                return render_template('index.html', message=message)

            predicted_disease = get_predicted_value(user_symptoms)

            # Move the assignment of precautions here
            dis_des, precautions, medications, rec_diet, workout = helper(predicted_disease)

            # Check if precautions list is not empty before iterating
...
# Other routes (about, contact, developer, blog)...
@app.route('/about')
def about():
...
@app.route('/meal')
def meal():
    return render_template("mealplaner.html")

@app.route('/wgen')
def wgen():
    return render_template("workoutgenerator.html")
...
@app.route('/Mentalhealth')
def send_mentalhealth():
    return send_from_directory('static', 'Mentalhealth.png')

@app.route('/Fitness')
def send_fitness():
    return send_from_directory('static', 'Fitness.jpg')

@app.route('/Mealplanner')
def send_mealplanner():
    return send_from_directory('static', 'Mealplanner.jpg')
...
@app.route('/Nutrition')
def send_nutrition():
    return send_from_directory('static', 'Nutrition.jpeg')


@app.route('/about')
def send_about():
    return send_from_directory('static', 'about.svg')


@app.route('/contact')
def send_contact():
    return send_from_directory('static', 'contact.jpg')
